<?php
	...
	switch ($act) {
		case 'login':
			$uname = filter_var((filter_var($_POST['email'], FILTER_SANITIZE_EMAIL)), FILTER_VALIDATE_EMAIL);
			$pass = filter_var($_POST['password'], FILTER_SANITIZE_STRING);
			//$upass = password_hash(filter_var($_POST['upassword'], FILTER_SANITIZE_STRING), PASSWORD_BCRYPT);

			try{
				$query = $conn->prepare('SELECT * FROM users WHERE email = ?');
				$fin = $query->execute([$uname]);
				...
				if (($row = $query->fetch()) && ($pass == $row['password'])) {
					...
					$page = "../../dashboard.php";
					$_SESSION['msg'] = $msg = "Welcome " . $_SESSION['username'];
		...
		case 'addUser':
			...
			$confirmpass = filter_var($_POST['password2'], FILTER_SANITIZE_STRING);
			...
			if ($password == $confirmpass) {
				try{
					...
					if ($existtrans) {
						...
						$conn->prepare($query)->execute([$username, $email, $phonenumber, $password]);
		...
		case 'addContribution':
			//
			$username = $_SESSION['username'];
			$planpackage = filter_var($_POST['planpackage'], FILTER_SANITIZE_STRING);
			$adultrate = filter_var($_POST['adultrate'], FILTER_SANITIZE_STRING);
			$childrate = filter_var($_POST['childrate'], FILTER_SANITIZE_STRING);
			$benefitslimit = filter_var($_POST['benefitslimit'], FILTER_SANITIZE_STRING);
			$benchmarks = filter_var($_POST['benchmarks'], FILTER_SANITIZE_STRING);

			try{
				$query = $conn->prepare('SELECT * FROM contributions WHERE Package = ?');
				$res = $query->execute(['$planpackage']);
				$existtrans = $query->fetch();

				if ($existtrans) {
					//user exists
					$_SESSION['msg'] = "A user account associated with the supplied plan name  exists.";
					$_SESSION['alertcolor'] = "danger";
					$source = $_SERVER['HTTP_REFERER'];
					header('Location: ' . $source);
				}
					...
					$query = 'INSERT INTO `contributions` (`Package`, `AdultRate`, `ChildRate`, `BenefitLimit`, `Benchmark`, `UpdatedBy`) VALUES (?,?,?,?,?,?)';
					$conn->prepare($query)->execute([$planpackage, $adultrate, $childrate, $benefitslimit, $benchmarks, $username]);

					$_SESSION['msg'] = $msg = 'Contributions Successfully Created';
					$_SESSION['alertcolor'] = $type = 'success';
					$source = $_SERVER['HTTP_REFERER'];
					header('Location: ' . $source);
				}
		...
		case 'addGlobalLimit':
			...
			$username = $_SESSION['username'];
			$planpackage = filter_var($_POST['planpackage'], FILTER_SANITIZE_STRING);
			$hospitalization = filter_var($_POST['hospitalization'], FILTER_SANITIZE_STRING);
			$pharmaceuticals = filter_var($_POST['pharmaceuticals'], FILTER_SANITIZE_STRING);
			$dental = filter_var($_POST['dental'], FILTER_SANITIZE_STRING);
			$optical = filter_var($_POST['optical'], FILTER_SANITIZE_STRING);
			$psychiatric = filter_var($_POST['psychiatric'], FILTER_SANITIZE_STRING);
			$rehabilitative = filter_var($_POST['rehabilitative'], FILTER_SANITIZE_STRING);
			$prosthetics = filter_var($_POST['prosthetics'], FILTER_SANITIZE_STRING);
			$radiology = filter_var($_POST['radiology'], FILTER_SANITIZE_STRING);
			$hearingaids = filter_var($_POST['hearingaids'], FILTER_SANITIZE_STRING);
			$refractionfees = filter_var($_POST['refractionfees'], FILTER_SANITIZE_STRING);
			$ancillaryservices = filter_var($_POST['ancillaryservices'], FILTER_SANITIZE_STRING);
			$privateambulances = filter_var($_POST['privateambulances'], FILTER_SANITIZE_STRING);
			$disabledhomes = filter_var($_POST['disabledhomes'], FILTER_SANITIZE_STRING);

			try{
				$query = $conn->prepare('SELECT * FROM globallimits WHERE Package = ?');
				$res = $query->execute(['$planpackage']);
				$existtrans = $query->fetch();
				...
				if ($existtrans) {
					...
					$query = 'INSERT INTO `globallimits` (`Package`, `Hospitalization`, `Pharmaceutical`, `Optical`, `Dental`, `Psychiatric`, `Rehabilitative`, `Prosthetics`, `Radiology`, `HearingAids`, `RefractionFees`, `AncillaryServices`, `PrivateAmbulance`, `DisabledHomes`, `UpdatedBy`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
					$conn->prepare($query)->execute([$planpackage, $hospitalization, $pharmaceuticals, $optical, $dental, $psychiatric, $rehabilitative, $prosthetics, $radiology, $hearingaids, $refractionfees, $ancillaryservices, $privateambulances, $disabledhomes, $username]);

					$_SESSION['msg'] = $msg = 'Global Limits Successfully Created';
					$_SESSION['alertcolor'] = $type = 'success';
					$source = $_SERVER['HTTP_REFERER'];
					header('Location: ' . $source);
				}
		...
		case 'addExchangeRate':
			...
			$isocode = filter_var($_POST['isocode'], FILTER_SANITIZE_STRING);
			$rate = filter_var($_POST['rate'], FILTER_SANITIZE_STRING);

			try{
				$query = $conn->prepare('SELECT * FROM rates WHERE Isocode = ?');
				$res = $query->execute(['$isocode']);
				$existtrans = $query->fetch();

				if ($existtrans) {
					//user exists
					$_SESSION['msg'] = "A user account associated with the supplied isocode  exists.";
					$_SESSION['alertcolor'] = "danger";
					$source = $_SERVER['HTTP_REFERER'];
					header('Location: ' . $source);
				}
					...
					$query = 'INSERT INTO `rates`(`Currency`, `Isocode`, `Rate`, `UpdatedBy`) VALUES (?,?,?,?)';
					$conn->prepare($query)->execute([$currency, $isocode, $rate, $username]);

					$_SESSION['msg'] = $msg = 'Exchange Rate Successfully Created';
					$_SESSION['alertcolor'] = $type = 'success';
					$source = $_SERVER['HTTP_REFERER'];
					header('Location: ' . $source);
				}
